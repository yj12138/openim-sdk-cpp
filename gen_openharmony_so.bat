...
@REM set GOARCH=arm
@REM set CXXFLAGS=--target=arm-linux-ohos -march=armv7-a -mfloat-abi-softfp -generic-armv7-a -mthumb %BASE_FLAGS%
@REM set CFLAGS=--target=arm-linux-ohos %BASE_FLAGS%
@REM set CC=%NDK_PATH%\llvm\bin\clang %CFLAGS% %BASE_FLAGS%
@REM set CXX=%NDK_PATH%\llvm\bin\clang++ %CFLAGS% %BASE_FLAGS%
@REM  not support 
